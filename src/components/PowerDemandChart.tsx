...
const levels = {
 ...
 month: {
   ...
   points: 12,
   getLabel: (i: number) => new Date(0, i).toLocaleString("default", { month: "short" }),
 },
};
...
const PowerDemandChart: FC = () => {
 ...
 const { labels, maxDemand, energyUsed } = useMemo(() => {
   ...
   const labels = Array.from({ length: config.points }, (_, i) => config.getLabel(i));
   const maxDemand = Array.from({ length: config.points }, () => Math.floor(Math.random() * 80 + 120)); // 120–200 kW
   const energyUsed = Array.from({ length: config.points }, () => Math.floor(Math.random() * 800 + 1200)); // 1200–2000 kWh
 ...
 return (
   <div className="bg-white rounded-2xl p-4 shadow-md w-full">
     <div className="flex justify-between items-center mb-4">
       <h2 className="text-xl font-semibold">Power Demand & Energy Use</h2>
       <select
